...
import { ApTextArea, ApTextBox } from "./ap_textbox";

const CONTACT_EVENT = "apex:open-contact";

export function openApContact() {
  window.dispatchEvent(new CustomEvent(CONTACT_EVENT));
}

export default function ApContactModal() {
        ...
        {status === "sent" ? (
          ...
          <>
            ...
            <form onSubmit={submit} className="contact-form" noValidate>
              <label>Name<ApTextBox required name="name" minLength={2} maxLength={80} autoComplete="name" /></label>
              <label>Work email<ApTextBox required type="email" name="email" autoComplete="email" /></label>
              <label className="field-wide">Company<ApTextBox name="company" maxLength={120} autoComplete="organization" /></label>
              <label className="field-wide">What are you trying to improve?<ApTextArea required name="message" minLength={10} maxLength={2000} rows={5} /></label>
              {status === "error" && <p className="form-error field-wide">{error}</p>}
              <button className="button button-primary field-wide" type="submit" disabled={status === "sending"}>{status === "sending" ? "Sending…" : "Send request"}<ApIcon name="arrow-up-right" /></button>
            </form>
          </>
        )}
      </div>
