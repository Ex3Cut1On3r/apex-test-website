...
import { Icon } from "./ap_admin_icons";

export default function MediaManager({ referencedPaths }: { referencedPaths: string[] }) {
    ...
    <div className="media-grid">
      <section className="panel">
        ...
        <div style={{ padding: 18 }}>
          <div className="upload-zone">
            <div className="metric-icon" style={{ margin: "0 auto 14px", width: 44, height: 44, borderRadius: 12, display: "grid", placeItems: "center", background: "#e9f7f5", color: "#078b81" }}><Icon name="upload" /></div>
      ...
      <section className="panel">
        ...
        <div className="path-list">
          {referencedPaths.map((path) => <div className="path-item" key={path}><code>{path}</code><button className="ap-button ap-button-soft" style={{ minHeight: 34 }} onClick={() => copy(path)}><Icon name="link" /> Copy</button></div>)}
        </div>
      </section>
    </div>
  );
