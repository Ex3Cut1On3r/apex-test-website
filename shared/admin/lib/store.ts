...
function localePath(locale: Locale) {
  return path.join(process.cwd(), "shared", "content", `${locale}.json`);
}
...
function githubContentPath(locale: Locale) {
  if (locale === "ar") return process.env.GITHUB_CONTENT_PATH_AR || "shared/content/ar.json";
  return process.env.GITHUB_CONTENT_PATH_EN || "shared/content/en.json";
}
...
async function saveToGithub(locale: Locale, content: SiteContent): Promise<SaveResult> {
  const repo = process.env.GITHUB_REPO!;
  const branch = process.env.GITHUB_BRANCH || "main";
  const contentPath = githubContentPath(locale);
...
export async function saveContent(locale: Locale, content: SiteContent): Promise<SaveResult> {
...
export async function uploadMedia(file: File) {
  ...
  if (githubReady()) {
    const repo = process.env.GITHUB_REPO!;
    const branch = process.env.GITHUB_BRANCH || "main";
    const root = (process.env.GITHUB_MEDIA_PATH || "public/shared/assets/cms").replace(/^\/+|\/+$/g, "");
